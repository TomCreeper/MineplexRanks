...
public class MapLead implements CommandExecutor{
    @Override
    public boolean onCommand(CommandSender sender, Command cmd, String s, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            if (args.length >= 3)
            {
                try
                {
                    Bukkit.broadcastMessage(Main.getLevelColor(Integer.parseInt(args[1])) + ChatColor.DARK_PURPLE + ChatColor.BOLD + " MAPLEAD " + ChatColor.YELLOW + args[0] + " " + ChatColor.WHITE + StringUtils.join(args, " ", 2, args.length));
                }
            ...
            {
                p.sendMessage(ChatColor.DARK_RED + "" + ChatColor.BOLD + "ERROR " + ChatColor.RED + "Incorrect usage. /maplead <name> <level> <message>");
            }
        }
        else
        {
