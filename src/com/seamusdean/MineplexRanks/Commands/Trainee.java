...
public class Trainee implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String s, String[] args)
    {
        if (sender instanceof Player)
        {
            ...
            if (args.length >= 3)
            {
                try
                {
                    Bukkit.broadcastMessage(Main.getLevelColor(Integer.parseInt(args[1])) + ChatColor.DARK_AQUA + ChatColor.BOLD + " TRAINEE " + ChatColor.YELLOW + args[0] + " " + ChatColor.WHITE + StringUtils.join(args, " ", 2, args.length));
                }
            ...
            {
                p.sendMessage(ChatColor.DARK_RED + "" + ChatColor.BOLD + "ERROR " + ChatColor.RED + "Incorrect usage. /owner <name> <level> <message>");
            }
        }
        else
        {
